// Subscribe to event hub "thumbnailevent" and process the media stored in Azure Blob Storage by firing up an FFMPEG docker container, and then send a "joinevent" to the Event Hub
'use strict';
...
// Name of the "Thumbnail" Event 
var incomingEventHubPath = 'thumbnailevent';
...
var outgoingEventHubPath = 'joinevent';
...
// Worker Interface: We will listen for the "thumbnailevent" on Event Hub to trigger the creation of an FFMPEG container and then get the incoming media file from Azure Blob storage, process it, and then store it back in Azure Blob Storage under a new name
...
docker.image.create({}, { fromImage: 'jrottenberg/ffmpeg', tag: 'latest' })
    ...
    .then(events => {
        ...
        // Open the Event Hub client so we can listen to the thumbnailevent events
                ...
                return partitionIds.map(function (partitionId) {
                    return incomingEventHubClient.createReceiver('$Default', '0', { startAfterTime: Date.now() }).then(rx => {
                        ...
                        rx.on('message', (message, test) => {
                            ...
                            // Ok, we got an incoming thumbnailevent
                            ...
                            blobSvc.getBlobToLocalFile(blobContainerName, fileName, '/tmp/' + fileName, (error, result, response) => {
                                if (!error) {
                                    ...
                                    // The destination file will have a postfix _t, indicating it is a thumbnail
                                    var destinationFileName = fileName.replace('.', '_t.');
                                    ...
                                    return docker.container.create({
                                        ...
                                        // This FFMPEG command line will take the source file straight from the Azure Blob Storage and create a 120px width thumbnail of it
