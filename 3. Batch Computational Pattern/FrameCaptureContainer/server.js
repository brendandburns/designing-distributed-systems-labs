// Subscribe to event hub "framecaptureevent" and process the media stored in Azure Blob Storage by firing up an FFMPEG docker container, and then send a "framecapturereadyevent" to the Event Hub
'use strict';
...
var incomingEventHubPath = 'framecaptureevent';
...
var outgoingEventHubPath = 'framecapturereadyevent';
...
// Worker Interface: We will listen for the "framecapture" on Event Hub to trigger the creation of an FFMPEG container and then get the incoming media file from Azure Blob storage, process it, and then store it back in Azure Blob Storage under a new name
...
docker.image.create({}, { fromImage: 'jrottenberg/ffmpeg', tag: 'latest' })
    ...
    .then(events => {
        ...
        // Open the Event Hub client so we can listen to the framecaptureevent events
            ...
            .then(function (partitionIds) {
                ...
                return partitionIds.map(function (partitionId) {
                    return incomingEventHubClient.createReceiver('$Default', '0', { startAfterTime: Date.now() }).then(rx => {
                        ...
                        rx.on('message', (message, test) => {

                            //TODO: Make this Azure Blob Storage triggered...

                            // Ok, we got an incoming framecapturelevent
                            ...
                            blobSvc.getBlobToLocalFile(blobContainerName, fileName, '/tmp/' + fileName, (error, result, response) => {
                                if (!error) {
                                    ...
                                    // The destination file will have a postfix png
                                    var destinationFileName = fileName.substr(0, fileName.lastIndexOf('.')) + '_f.png';
                                    ...
                                    return docker.container.create({
                                        ...
                                        // This command line will take the source file straight from the Azure Blob Storage through a public endpoint, take 1 single frame at 5 seconds into the view, and output a HD sized frame capture of it and store it locally in the FFMPEG container's /tmp folder
                                        Cmd: ['-y', '-i', fileUrl, '-ss', '00:00:05', '-t', '00:00:01', '-r', '1', '-vf', 'scale=1920:-1', '/tmp/' + destinationFileName],
                                        AttachStdout: true,
                                        AttachStderr: true,
                                        Detach: false
